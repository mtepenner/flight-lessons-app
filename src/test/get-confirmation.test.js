...
describe("get-confirmation Netlify function", () => {
  ...
  it("returns only the generated confirmation text", async () => {
      ...
      .mockResolvedValueOnce({
        ...
        json: vi.fn().mockResolvedValue({
          content: [
            {
              type: "text",
              text: "Cleared for takeoff, Amelia. We will see you on Thursday at 01:00 PM – 04:00 PM! ✈️",
            },
          ],
        }),
      });
    ...
    expect(JSON.parse(response.body)).toEqual({
      message:
        "Cleared for takeoff, Amelia. We will see you on Thursday at 01:00 PM – 04:00 PM! ✈️",
    });
  });
  ...
  it("rejects invalid flight days before calling Claude", async () => {
    process.env.CLAUDE_API_KEY = "test-key";
    process.env.SUPABASE_URL = "https://example.supabase.co";
    process.env.SUPABASE_ANON_KEY = "anon-key";
